...
describe('spriteFrameStyle', () => {
  it('scales to cover a square panel and centres horizontally', () => {
    // 380x380 panel: scale = max(380/56, 380/61) = 6.7857…
    const { backgroundSize, backgroundPosition } = spriteFrameStyle(380, 380, 0);
    const scale = Math.max(380 / 56, 380 / 61);
    const bw = 56 * scale;
    const bh = 61 * scale;
    expect(backgroundSize).toBe(`${68 * bw}px ${bh}px`);
    expect(backgroundPosition).toBe(`${(380 - bw) / 2}px ${(380 - bh) * 0.1}px`);
  });

  it('steps left by one frame width per frame', () => {
    const f0 = spriteFrameStyle(280, 280, 0);
    const f1 = spriteFrameStyle(280, 280, 1);
    const scale = Math.max(280 / 56, 280 / 61);
    const bw = 56 * scale;
  ...
  it('offsets vertically by 10% of the overflow, not by half', () => {
    ...
    const scale = Math.max(200 / 56, 400 / 61);
    const bh = 61 * scale;
    expect(parseFloat(backgroundPosition.split(' ')[1])).toBeCloseTo((400 - bh) * 0.1, 6);
  });
