...
/** Cover-fit one frame in a container, anchored to the top so the head is
 *  never cropped; any vertical overflow comes off the bottom. The scale is rounded
...
export function spriteFrameStyle(cw: number, ch: number, frame: number) {
  ...
  return {
    ...
    backgroundPosition: `${offX - frame * bw}px 0px`,
  };
}
